    ...
    getTicketInfo = (id) => {
        getTicket(id)
            .then((res) => {
                var temp = []
                temp.push(
                    `${mapFiledToChinese['title']}: ${res.message['title']}`
                )
                temp.push(
                    `${mapFiledToChinese['type']}: ${res.message['type']}`
                )
                temp.push(
                    `${mapFiledToChinese['status']}: ${res.message['status']}`
                )
                temp.push(`内容描述: ${res.message['describe']}`)
                // temp.push(`${mapFiledToChinese['describe']}: ${res.message['describe']}`)
                temp.push(
                    `${mapFiledToChinese['user']}: ${res.message['user']}`
                )
                temp.push(
