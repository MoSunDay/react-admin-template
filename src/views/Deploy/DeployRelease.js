...
class DeployRelease extends Component {
  ...
  componentDidMount() {
    ...
    response
      .then((res) => {
        this.setState({source: serviceVersion, serviceVersionList: res["content"], serviceName: service, serviceVersion: res["content"][0]});
      })
      .catch((err) => {
        console.log(err)
      });
        ...
        <Card title="发布服务" extra={this.goBack()}>
          ...
          >
            ...
            <Form.Item
              ...
              >
              <Col span={8}>
                <Input size="middle" placeholder={this.state.branch} maxLength="100"/>
              </Col>
            </Form.Item>
            <Form.Item>
